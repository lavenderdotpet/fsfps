...
//----------------------------------------------------------------------
	...
	.align 4
.globl C(R_DrawSurfaceBlock8GREEN_mip0)
C(R_DrawSurfaceBlock8GREEN_mip0):
...
//		{
	movl	C(r_lightptr_g),%ebx
	movl	C(r_numvblocks),%eax

	movl	%eax,sb_v
	movl	C(prowdestbasegreen),%edi

	movl	C(pbasesourcegreen),%esi
...
//			lightdelta = (lightleft - lightright) & 0xFFFFF;
	...
	movl	%eax,%ebp
	movl	C(r_lightwidther),%ecx
	
	movl	%edx,C(lightright)
	subl	%edx,%ebp
	leal	(%ecx + 2*%ecx),%ecx		// leilei
...
//----------------------------------------------------------------------
	...
	.align 4
.globl C(R_DrawSurfaceBlock8GREEN_mip1)
C(R_DrawSurfaceBlock8GREEN_mip1):
	pushl	%ebp				// preserve caller's stack frame
	pushl	%edi
	pushl	%esi				// preserve register variables
	pushl	%ebx
...
//----------------------------------------------------------------------
	...
	.align 4
.globl C(R_DrawSurfaceBlock8GREEN_mip2)
C(R_DrawSurfaceBlock8GREEN_mip2):
	pushl	%ebp				// preserve caller's stack frame
	pushl	%edi
	pushl	%esi				// preserve register variables
	pushl	%ebx
...
//----------------------------------------------------------------------
	...
	.align 4
.globl C(R_DrawSurfaceBlock8GREEN_mip3)
C(R_DrawSurfaceBlock8GREEN_mip3):
	pushl	%ebp				// preserve caller's stack frame
	pushl	%edi
	pushl	%esi				// preserve register variables
	pushl	%ebx
...
LPatchTable8:
	...
	.align 4
.globl C(R_Surf8PatchGREEN)
C(R_Surf8PatchGREEN):
	pushl	%ebx

	movl	C(host_colormap_green),%eax
