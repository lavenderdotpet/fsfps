...
// to try to use WinDirect modes even on NT
	...
	if (WinNT)
		useDirectDraw = false; // this sucks on nt

	
	if (COM_CheckParm("-nodirectdraw") || COM_CheckParm("-noddraw") || COM_CheckParm("-nodd"))
		useDirectDraw = false;
	if (COM_CheckParm("-directdraw") || COM_CheckParm("-ddraw") || COM_CheckParm("-dd"))
		useDirectDraw = true;
	...
	// build the mode list, leaving room for the low-res stretched mode, if any
		...
		if (lowstretched == 2)
		{
			...
			modelist[MODE_FULLSCREEN_DEFAULT].stretched = 2;
			modelist[MODE_FULLSCREEN_DEFAULT].width >>= 1;
			sprintf (modelist[MODE_FULLSCREEN_DEFAULT].modedesc, "%dx%dtwk",
					 modelist[MODE_FULLSCREEN_DEFAULT].width,
					 modelist[MODE_FULLSCREEN_DEFAULT].height);
		}

		else if (lowstretched)
		{
			modelist[MODE_FULLSCREEN_DEFAULT] = modelist[stretchedmode];
			modelist[MODE_FULLSCREEN_DEFAULT].stretched = 1;
			modelist[MODE_FULLSCREEN_DEFAULT].width >>= 1;
			modelist[MODE_FULLSCREEN_DEFAULT].height >>= 1;
			sprintf (modelist[MODE_FULLSCREEN_DEFAULT].modedesc, "%dx%dstr",
					 modelist[MODE_FULLSCREEN_DEFAULT].width,
					 modelist[MODE_FULLSCREEN_DEFAULT].height);
		}
...
{
	// 12/01/15 LEILEI - new Aspect hack
	// If we are using something not wide screen, stick to the old aspect
	
	if (COM_CheckParm ("-169")){
		vid.aspect = ((float)vid.height / (float)vid.width) *
				(1280.0 / 720.0);
	}
	else if (COM_CheckParm ("-1610")){
		vid.aspect = ((float)vid.height / (float)vid.width) *
				(1280.0 / 800.0);
	}
	else  if (COM_CheckParm ("-43"))
	{
		vid.aspect = ((float)vid.height / (float)vid.width) *
				(320.0 / 240.0);
}
	...
	else
	{
		...
		//	else 
		//		vid.aspect = ((float)vid.height / (float)vid.width) * ((float)vid.height / (float)vid.width);
				//vid.aspect = ((float)vid.height / (float)vid.width) * (1280 /  720);
...
// so Con_Printfs don't mess us up by forcing vid and snd updates
	...
	if (vid_stretched == 2){
	window_width = vid.width << 1;
	window_height = vid.height;

	}
	else if (vid_stretched == 3){
	window_width = vid.width;
	window_height = vid.height << 1;
...
void	VID_Init (unsigned char *palette)
{
	...
	if (COM_CheckParm ("-dibonly")) // leilei says FORCE DIBONLY BECAUSE MGL IS CRAAAAAP
		dibonly = true;

	if (WinNT)
		dibonly = true;	// nt suks
	if (COM_CheckParm("-directdraw") || COM_CheckParm("-ddraw") || COM_CheckParm("-dd"))
		dibonly = false;
