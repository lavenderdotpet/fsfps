...
//	SZ_Alloc (&cmd_text, 8192);		// space for commands and script files
	SZ_Alloc (&cmd_text, 32*1024);		// space for commands and script files
...
// copy off any commands still remaining in the exec buffer
	...
	if (templen)
	{
		temp = Z_Malloc (mainzone, templen);	// 2001-09-20 Enhanced zone handling by Maddes
...
// add the copied off data
	if (templen)
	{
		...
		Z_Free (mainzone, temp);	// 2001-09-20 Enhanced zone handling by Maddes
	}
}
...
// build the combined string to parse from
	...
	text = Z_Malloc (mainzone, s+1);	// 2001-09-20 Enhanced zone handling by Maddes
	text[0] = 0;
	for (i=1 ; i<com_argc ; i++)
	{
		if (!com_argv[i])
...
// pull out the commands
	build = Z_Malloc (mainzone, s+1);	// 2001-09-20 Enhanced zone handling by Maddes
	build[0] = 0;
	...
	Z_Free (mainzone, text);	// 2001-09-20 Enhanced zone handling by Maddes
	Z_Free (mainzone, build);	// 2001-09-20 Enhanced zone handling by Maddes
}
...
char *CopyString (char *in)
{
	...
	out = Z_Malloc (mainzone, strlen(in)+1);	// 2001-09-20 Enhanced zone handling by Maddes
	strcpy (out, in);
	return out;
}
...
void Cmd_Alias_f (void)
{
	...
	for (a = cmd_alias ; a ; a=a->next)
	{
		...
		{
			Z_Free (mainzone, a->value);	// 2001-09-20 Enhanced zone handling by Maddes
			break;
		}
	}
	...
	if (!a)
	{
		a = Z_Malloc (mainzone, sizeof(cmdalias_t));	// 2001-09-20 Enhanced zone handling by Maddes
		a->next = cmd_alias;
		cmd_alias = a;
	}
	strcpy (a->name, s);
