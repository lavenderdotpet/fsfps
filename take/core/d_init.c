...
void D_SetupFrame (void)
{
	int		i;
		...
		//r_lowdetail = (int)vid_low->value;

	if (r_dowarp)
		d_viewbuffer = r_warpbuffer;
	//else if (vid_loww)
		//d_viewbuffer = r_warpbuffer;  // BUG - this causes a nice crash. Yay.
	else
		d_viewbuffer = (void *)(byte *)vid.buffer;


		//d_viewbuffer = r_lowdbuffer;

	if (r_dowarp)
		screenwidth = WARP_WIDTH;
	//else if (vid_loww)
		//screenwidth = vid.width / 2;  // BUG -s crews up guns
	else
