...
/*		if (vid_low->value || r_viewchanged || lcd_x->value)
		{
...
/*
	vid_lowold = vid_loww;
	vid_loww = vid_low->value;

	if ((vid_loww != vid_lowold) || r_viewchanged || lcd_x->value)
	{
		if (vid_loww)
		{
			if ((vid.width <= (vid.width / 2)) &&
				(vid.height <= (vid.height / 2)))
			{
				vrect.x = 0;
				vrect.y = 0;
				vrect.width = vid.width;
