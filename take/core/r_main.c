...
void R_Init_Cvars (void)
{
	...
	vid_low = Cvar_Get ("vid_low", "0", CVAR_ORIGINAL);

//	r_lowdetail = Cvar_Get ("r_lowdetail", "1", CVAR_ORIGINAL);
	...
	r_dyndelay = Cvar_Get ("r_dyndelay", "0", CVAR_ARCHIVE|CVAR_ORIGINAL);
...
void R_RenderView_ (void)
{
	...
	byte	lowdbuffer[WARP_WIDTH * WARP_HEIGHT];

	r_warpbuffer = warpbuffer;
	r_lowdbuffer = lowdbuffer;
